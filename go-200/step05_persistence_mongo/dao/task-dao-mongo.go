...
func (s *TaskDAOMongo) getAllTasksByQuery(query interface{}, start, end int) ([]model.Task, error) {
	...
	if hasPaging {
		// TODO use the options.Find() method with Skip() and Limit() (to be calculated) parameters to retrieve all the results
		// TODO TIP : if start == end, limit must be one
	}

	// find query
	// TODO build the Find request, with a TODO context, the query and the opt
	// TODO handle Find error

	// defer close cursor
	defer func() {
		// TODO close the cursor when done
		if err != nil {
	...
	// marshall results
	// TODO use the cursor to marshall all the tasks in the slice
	// TODO handle error
